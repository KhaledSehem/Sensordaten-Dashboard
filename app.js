document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchSensors() {
        try {
            const sensorsResponse = await fetch('http://localhost:3000/sensors');
    ...
    fetchSensors();
    intervalId = setInterval(fetchSensors, 60000); // Update sensor list every 60 seconds
    ...
    async function fetchSensorData() {
        ...
        try {
            ...
            // After all data is fetched, generate the table and render the chart
            if (allData.length > 0) {
            ...
            async function fetchSensorDetails(sensorId) {
                const url = new URL(`http://localhost:3000/sensor-data/${sensorId}`);
